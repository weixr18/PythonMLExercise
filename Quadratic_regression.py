...
def quadratic_reg(X, Y, d, N, X_t=[], Y_t=[]):
    """
    二次回归                     \n
    X: 样本集, [N*d]             \n
    Y: 标签集, [N]              \n
    ...
    w = (np.random.rand(d+1, d+1) - 0.5) / 10
    log = []

    for j in range(ROUND):
        # gradient decent (sample by sample)
        for x, y in zip(X, Y):
            dL_dw = (x.dot(w).dot(x) - y) * np.array([x]).T * x
            w -= dL_dw * LR
        ...
        if j % 100000 == 0:
            ...
            Loss = np.array([x.dot(w).dot(x) for x in X_val])
...
if __name__ == "__main__":
    ...
    # train
    w = quadratic_reg(X, Y, 4, len(X), X_t, Y_t)
